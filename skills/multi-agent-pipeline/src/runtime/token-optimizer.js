...
function buildToolSummary(selectedTools, excludedTools, estimatedSchemaTokenSavings) {
  ...
  return `Selected ${selectedNames.length} MCP tool(s), excluded ${excludedNames.length} tool(s), saving about ${estimatedSchemaTokenSavings} schema token(s).`;
}
...
export function recommendMcpToolSubset({
  toolDefinitions,
  objective,
  ...
  const selectedTools = [
    ...coreNames.map((name) => toolsByName.get(name) ?? {
      name,
      title: name,
      description: "Core MCP pipeline tool.",
    }),
...
export function buildTokenOptimizationReport({
  ...
  const toolSubset = recommendMcpToolSubset({
    toolDefinitions,
    objective,
    coreToolNames,
  });
